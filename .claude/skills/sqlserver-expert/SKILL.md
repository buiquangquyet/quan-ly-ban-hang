...
description: >-
  Expert SQL Server development và administration. Dùng khi viết T-SQL queries,
  stored procedures, functions, triggers, tối ưu performance (deadlocks, slow queries,
  execution plans, index tuning), thiết kế schema, cấu hình SQL Server, implement CDC
  (Change Data Capture), hoặc tích hợp SQL Server với .NET Core/C# qua Entity Framework
  Core hoặc Dapper. Cũng dùng khi user hỏi "query chậm", "fix deadlock", "optimize query",
  "design table", hoặc nhắc đến tempdb, query plan, wait stats, SQL Server error messages.
---
