...
After every API call in a `When` step, **always** attach the response to the Cucumber HTML report. This makes failures self-documenting — no need to re-run with extra logging.
