...
> **Rule:** After every `When` step that calls an API, attach the response with `await this.attach(JSON.stringify(response, null, 2), 'text/plain')`. This makes the Cucumber HTML report self-documenting — the response body is visible directly in the report for every scenario, making failures much easier to diagnose without re-running tests.
